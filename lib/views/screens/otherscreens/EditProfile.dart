...
class EditProfileScreen extends StatefulWidget {
  ...
  static var routeName = 'edit-profile-screen';
...
class _EditProfileScreenState extends State<EditProfileScreen> {
  ...
  void getUserDetails() async {
    ...
    var user = await userProvider.getUser(authId).catchError((e) {
    ...
    if (user != null && user.id.toString().isNotEmpty) {
      ...
      var number =
          PhoneNumber.fromCompleteNumber(completeNumber: user['PhoneNo']);
      ...
      for (int i = 0; i < genders.length; i++) {
        if (genders[i].name == user['Gender']) {
          genders[i].isSelected = true;
          gender = genders[i].name;
          break;
        }
      }
      userCreationTime = user['CreatedAt'];
  ...
  Future updateUser(BuildContext ctx) async {
    var userProvider = Provider.of<UserProvider>(context, listen: false);
    var authId = Provider.of<Auth>(context, listen: false).token;
    ...
    if (isValid) {
      await userProvider
        ...
        UserModel(
          ...
          dob: date,
          gender: gender,
          createdAt: userCreationTime.toDate(),
          isCreator: isCreator,
        ),
      )
          .catchError((e) {
